...
def edit_tags(file_path, title=None, artist=None, album=None, year=None, genre=None, track=None, cover_path=None):
    path = Path(file_path)
    ext = path.suffix.lower()
    if ext == ".mp3": _edit_mp3(path, title, artist, album, year, genre, track, cover_path)
    elif ext == ".flac": _edit_flac(path, title, artist, album, year, genre, track, cover_path)
    elif ext in (".m4a", ".mp4", ".aac"): _edit_mp4(path, title, artist, album, year, genre, track, cover_path)
    else: raise ValueError(f"فرمت پشتیبانی نمی‌شه: {ext}")
    return True
